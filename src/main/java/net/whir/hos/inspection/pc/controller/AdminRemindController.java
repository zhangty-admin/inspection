...
public class AdminRemindController {
    ...
    private Result findPageAdminRemind(@RequestBody PageRequest<AdminRemind> remindPageRequest) {
        List<AdminRemind> unifiedRemindList = remindService.findPage(remindPageRequest);
        return new Result(true, StatusCode.OK, "查询成功", unifiedRemindList);
    }

    @ApiOperation(value = "新增提醒管理员")
    @PostMapping("/add")
    private Result addAdminRemind(@RequestBody AdminRemind adminRemind) {
        try {
