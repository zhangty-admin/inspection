...
public class InspectionHistoryController {
    ...
    private Result findPageInspection(@RequestBody PageRequest<InspectionHistory> inspectionHistoryPageResult) {
        PageResult pageResult = null;
        try {
            Page<InspectionHistory> page = historyInspectionService.findPage(inspectionHistoryPageResult);
   ...
   /* @ApiOperation(value = "历史巡检根据巡检ID分页检查项")
    ...
    private Result findItemsById(@RequestBody PageRequest<Long> pageRequest) {
        PageResult pageResult = null;
        try {
            Page<ItemEmployee> page = historyInspectionService.findItemsById(pageRequest);
            pageResult = new PageResult(page.getTotal(), page.getPages(), page.getPageNum(), page.getPageSize(), page.getResult());
        } catch (Exception e) {
            log.warn("分页查询失败: " + e.getMessage());
            return new Result(false, StatusCode.ERROR, "查询失败");
